...
function automaticSlide() {
  ...
  const indicator = document.getElementsByClassName("dot");
  for (pics = 0; pics < img.length; pics++) {
    img[pics].style.display = "none";
  }
  firstIndeex++;
  if (firstIndeex > img.length) {
    firstIndeex = 1;
  }
  for (pics = 0; pics < indicator.length; pics++) {
    indicator[pics].className = indicator[pics].className.replace(
      " active",
      ""
    );
  }
  img[firstIndeex - 1].style.display = "block";
  indicator[firstIndeex - 1].className += " active";
...
// Making a function For the Product list
function clotthing() {
  let holder = document.getElementsByClassName("holder");
  for (let i = 0; i < productList.length; i++) {
    let images = document.createElement("img");
    let paraGraphh = document.createElement("p");
    paraGraphh.style.color = "rgb(27,25,25)";
    paraGraphh.style.textTransform = "capitalize";
    paraGraphh.style.paddingLeft = "10px";
    paraGraphh.style.fontWeight = "400";
    paraGraphh.style.paddingTop = "10px";
    let para1 = document.createElement("p");
    para1.style.color = "rgb(110,110,110)";
    para1.style.textTransform = "500";
    para1.style.paddingLeft = " 10px";
    ...
    Price.style.textTransform = "Capitalize";
    Price.style.fontWeight = "600";
    paraGraphh.innerText = productList[i].name;
    para1.innerText = productList[i].brand;
    Price.innerText = "Rs " + productList[i].price;
    images.setAttribute("src", productList[i].preview);
    images.setAttribute("height", "300px");
    images.setAttribute("width", "100%");
    images.setAttribute("alt", "eror");
    holder[i].appendChild(images);
    holder[i].appendChild(paraGraphh);
    holder[i].appendChild(para1);
    holder[i].appendChild(Price);
  }
}
clotthing();
